...
tg__bingji:addEffect(fk.AfterCardsMove, {
  ...
  can_trigger = function(self, event, target, player, data)
    if not player:hasSkill(tg__bingji.name) then return false end
  ...
  on_trigger = function(self, event, target, player, data)
    ...
    for _, e in ipairs(events) do
      if not player:hasSkill(tg__bingji.name) or player.dead then break end
      if table.find(room.alive_players, function(p) return not table.contains(mark[e], p.id) end) then
        ...
        skill:doCost(event, target, player, e)
      end
    end
  end,
  on_cost = function(self, event, target, player, data)
    ...
    if #targets > 0 then
      event:setCostData(skill, targets[1].id)
      return true
    end
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    local mark = type(player:getMark("_tg__bingji")) == "table" and player:getMark("_tg__bingji") or { {}, {}, {} }
    local targetId = event:getCostData(skill)
...
local tg__bingji_maxcards = fk.CreateSkill {
  name = "#tg__bingji_maxcards"
}

tg__bingji_maxcards:addEffect('maxcards', {
