local tg__fenwei = fk.CreateSkill {
  name = "tg__fenwei"
}
...
tg__fenwei:addEffect(fk.CardUseFinished, {
  anim_type = "offensive",
  can_trigger = function(self, event, target, player, data)
    if target == player or not player:hasSkill(tg__fenwei.name) or player:usedSkillTimes(tg__fenwei.name) > 0 or table.find(player.room.alive_players, function(p)
      return p.dying
    end) then 
      return false 
    end
  ...
  on_use = function(self, event, target, player, data)
    player:turnOver()
    local choice = event:getCostData(self):split("_")[4]
    player.room:useVirtualCard(choice, nil, player, target, tg__fenwei.name, true)
  end,
})
