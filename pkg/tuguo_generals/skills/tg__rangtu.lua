...
biyue:addEffect(fk.EventPhaseStart, {
  ...
  can_trigger = function(self, event, target, player)
    return player:hasSkill(biyue.name) and target.phase == Player.Start and player ~= target and target.hp >= player.hp and not player:isKongcheng() and not target:isKongcheng()
  end,
  ...
  on_use = function(self, event, target, player)
    ...
    local pd = player:pindian({target}, biyue.name)
...
biyue:addEffect(fk.PindianCardsDisplayed, {
  anim_type = "negative",
  frequency = Skill.Compulsory,
  can_trigger = function(self, event, target, player)
    return player:getMark("@@tg__rangtu") > 0 and (target == player or table.contains(target.tos, player))
  end,
  on_use = function(self, event, target, player)
    player.room:setPlayerMark(player, "@@tg__rangtu", 0)
    if target.from == player then
      target.fromCard.number = math.max(target.fromCard.number - 2, 1)
    else
      target.results[player.id].toCard.number = math.max(target.results[player.id].toCard.number - 2, 1)
    end
  end,
})
