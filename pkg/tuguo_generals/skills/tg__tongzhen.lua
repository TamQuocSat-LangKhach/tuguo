...
tg__tongzhen:addEffect(fk.AfterCardsMove, {
  ...
  on_cost = function(self, event, target, player)
    ...
    table.insertIfNeed(mark, "tg__losehandcard")
...
tg__tongzhen:addEffect(fk.AfterDying, {
  can_trigger = function(self, event, target, player)
    if not player:hasSkill(tg__tongzhen.name, false, true) or player:prohibitUse(Fk:cloneCard("slash")) then return false end
    local room = player.room
  ...
  on_cost = function(self, event, target, player)
    ...
    if #availableTargets == 0 then return false end
    local mark = type(player:getMark("@tg__tongzhen")) == "table" and player:getMark("@tg__tongzhen") or {}
    table.insertIfNeed(mark, "tg__afterdying")
    local targets = room:askToChoosePlayers(player, {
      targets = availableTargets,
      min_num = 1,
      max_num = 99,
...
tg__tongzhen:addEffect(fk.Death, {
  can_trigger = function(self, event, target, player)
    if not player:hasSkill(tg__tongzhen.name, false, true) or player:prohibitUse(Fk:cloneCard("slash")) then return false end
    local room = player.room
  ...
  on_cost = function(self, event, target, player)
    ...
    if #availableTargets == 0 then return false end
    local mark = type(player:getMark("@tg__tongzhen")) == "table" and player:getMark("@tg__tongzhen") or {}
    table.insertIfNeed(mark, "tg__die")
    local targets = room:askToChoosePlayers(player, {
      targets = availableTargets,
      min_num = 1,
      max_num = 99,
...
tg__tongzhen:addEffect(fk.Deathed, {
  can_trigger = function(self, event, target, player)
    if not player:hasSkill(tg__tongzhen.name, false, true) or player:prohibitUse(Fk:cloneCard("slash")) then return false end
    local room = player.room
  ...
  on_cost = function(self, event, target, player)
    ...
    if #availableTargets == 0 then return false end
    local mark = type(player:getMark("@tg__tongzhen")) == "table" and player:getMark("@tg__tongzhen") or {}
    table.insertIfNeed(mark, "tg__lonearmy")
    local targets = room:askToChoosePlayers(player, {
      targets = availableTargets,
      min_num = 1,
      max_num = 99,
...
tg__tongzhen:addEffect(fk.GameStart, {
  can_trigger = function(self, event, target, player)
    if not player:hasSkill(tg__tongzhen.name, false, true) or player:prohibitUse(Fk:cloneCard("slash")) then return false end
    local room = player.room
    if player:getMark("_tg__tongzhen_lonearmy") == 1 then return false end
